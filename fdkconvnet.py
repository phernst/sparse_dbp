...
class Normalize:
    def __call__(_, sample):
        slist = [sample['sparse'], sample['img']]
        img_norm = NORMALIZATION['images_99']
        slist[0] = slist[0]/img_norm
...
class RandomVerticalFlip:
    def __call__(_, sample):
        ...
        return {
            ...
            'img': np.ascontiguousarray(slist[1]),
        }
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filename, plane: HilbertPlane,
        ...
        data_path = DATA_DIRS['fdk_36']
        ds_shape = nib.load(pjoin(data_path, filename))
    ...
    def _load_ds_if_none(self):
        ...
        if image_volume is None:
            image_volume = load_nib(
                pjoin(DATA_DIRS['datasets'], self.filename),
            )[..., None]
    ...
    def __getitem__(self, idx):
        self.sparse_volume, self.image_volume = self._load_ds_if_none()
...
class FDKConvNet(LightningModule):
    def __init__(self, **kwargs):
        ...
        self.example_input_array = torch.empty(1, 1, 512, 512)
    ...
    def training_step(self, batch, _):
        sparse, image = batch['sparse'], batch['img']
